...
sudo apt-get install -y ca-certificates curl gnupg # ca-certificates curl gnupg
sudo mkdir -p /etc/apt/keyrings # create directory for keyrings
curl -fsSL https://deb.nodesource.com/gpgkey/nodesource-repo.gpg.key | sudo gpg --dearmor -o /etc/apt/keyrings/nodesource.gpg # add key for nodejs

NODE_MAJOR=20 # set node major version
echo "deb [signed-by=/etc/apt/keyrings/nodesource.gpg] https://deb.nodesource.com/node_$NODE_MAJOR.x nodistro main" | sudo tee /etc/apt/sources.list.d/nodesource.list # add repo for nodejs

sudo apt-get update # update apt
sudo apt-get install nodejs -y # install nodejs latest version
