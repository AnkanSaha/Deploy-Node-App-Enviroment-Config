...
Ubuntu20.04NodeEnviromentSetup() {
    ...
    read "$YELLOW Do you want to Change MongoDB Port? (y/n): " mongoPORTchoice # ask user to change mongodb port or not
    ...
    if [ "$mongoPORTchoice" == "y" ] || [ "$mongoPORTchoice" == "Y" ]; then
        ...
        read "$YELLOW Do you want to Change MongoDB Bind IP? (y/n): " mongoBINDIPchoice    # ask user to change mongodb bind ip or not
        ...
        if [ "$mongoBINDIPchoice" == "y" ] || [ "$mongoBINDIPchoice" == "Y" ]; then
            ...
            echo "$MAGENTA Skipping MongoDB Bind IP Change."
        fi
    else
        echo "$MAGENTA Skipping MongoDB Port Change."
    fi
    # Tell User that MongoDB Config File is changed
    echo "$YELLOW Your New MongoDB IP is: $NORMAL $mongoBINDIP & $BOLD Your New MongoDB Port is: $NORMAL $mongoPORT"
